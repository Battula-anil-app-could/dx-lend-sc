...
import { AddressValue, BigIntValue, CodeMetadata, IAddress, Interaction, ResultsParser, ReturnCode, SmartContract, SmartContractAbi, Struct, TokenIdentifierValue, TokenPayment, TransactionWatcher } from "@dharitrinetwork/moajs";
import { IAudit, INetworkConfig, INetworkProvider, ITestSession, ITestUser, loadAbiRegistry, loadCode } from "@dharitrinetwork/moajs-snippets";
...
export class LendingPoolInteractor {
    ...
    async issueLend(user: ITestUser, tokenIdentifier: string): Promise<ReturnCode> {
        ...
        let interaction = <Interaction>this.contract.methods
            ...
            .withValue(TokenPayment.moaxFromAmount(0.05))
            .withNonce(user.account.getNonceThenIncrement())
            .withChainID(this.networkConfig.ChainID);
    ...
    async deposit(user: ITestUser, tokenPayment: TokenPayment): Promise<{ returnCode: ReturnCode, depositNonce: number }> {
        ...
        let interaction = <Interaction>this.contract.methods
            ...
            .withSingleDCTTransfer(tokenPayment)
            .withNonce(user.account.getNonceThenIncrement())
            .withChainID(this.networkConfig.ChainID);
    ...
    async withdraw(user: ITestUser, tokenPayment: TokenPayment): Promise<ReturnCode> {
        ...
        let interaction = <Interaction>this.contract.methods
            .withdraw([])
            .withGasLimit(15000000)
            .withSingleDCTNFTTransfer(tokenPayment, user.address)
            .withNonce(user.account.getNonceThenIncrement())
            .withChainID(this.networkConfig.ChainID);
    ...
    async borrow(user: ITestUser, collateralPayment: TokenPayment, assetToBorrow: string): Promise<{ returnCode: ReturnCode, borrowNonce: number }> {
        ...
        let interaction = <Interaction>this.contract.methods
            ...
            .withSingleDCTNFTTransfer(collateralPayment, user.address)
            .withNonce(user.account.getNonceThenIncrement())
            .withChainID(this.networkConfig.ChainID);
    ...
    async repay(user: ITestUser, repayment: TokenPayment[], assetToRepay: string): Promise<ReturnCode> {
        ...
        let interaction = <Interaction>this.contract.methods
            .repay([assetToRepay])
            .withGasLimit(150000000)
            .withMultiDCTNFTTransfer(repayment, user.address)
            .withNonce(user.account.getNonceThenIncrement())
            .withChainID(this.networkConfig.ChainID);

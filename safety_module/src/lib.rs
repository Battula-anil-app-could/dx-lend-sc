...
pub trait SafetyModule {
    #[init]
    fn init(&self, wmoax_token: TokenIdentifier, depositors_apy: BigUint) {
        self.wmoax_token().set(&wmoax_token);
    ...
    fn fund(self, caller: OptionalValue<ManagedAddress>) {
        ...
        require!(token == self.wmoax_token().get(), "invalid token");
        ...
        self.send()
            .direct_dct(&caller_address, &nft_token, 0, &payment);
    }
    ...
    fn fund_from_pool(&self) {
        ...
        self.convert_to_wmoax(token, payment);
    }
    ...
    fn take_funds(&self, pool_token: TokenIdentifier, amount: BigUint) {
        ...
        self.convert_wmoax(pool_token.clone(), amount.clone());

        self.send()
            .direct_dct(&caller_address, &pool_token, 0, &amount);
    }

    #[payable("*")]
    #[endpoint(withdraw)]
    fn withdraw(&self) -> BigUint {
        let (token_id, nft_nonce, amount) = self.call_value().single_dct().into_tuple();
        let caller_address = self.blockchain().get_caller();
        ...
        let wmoax_token_id = &self.wmoax_token().get();
        let contract_balance = self.blockchain().get_dct_balance(
            &self.blockchain().get_sc_address(),
            wmoax_token_id,
            0,
        );
        ...
        self.send()
            .direct_dct(&caller_address, wmoax_token_id, 0, &withdraw_amount);
    ...
    fn set_local_roles_nft_token(&self, roles: MultiValueEncoded<DctLocalRole>) {
    ...
    fn set_local_roles(&self, token: TokenIdentifier, roles: ManagedVec<DctLocalRole>) {
        self.send()
            .dct_system_sc_proxy()
    ...
    fn convert_wmoax(&self, _pool_token: TokenIdentifier, _amount: BigUint) {
        //TODO:  integration with dex
    }

    fn convert_to_wmoax(&self, _pool_token: TokenIdentifier, _amount: BigUint) {
        //TODO:  integration with dex
    }
    ...
    #[view]
    #[storage_mapper("wmoax_token")]
    fn wmoax_token(&self) -> SingleValueMapper<TokenIdentifier>;

...
issueBUSD(){
    erdpy contract call ${ESTD_ISSUE_ADDRESS} --value=5000000000000000000 --pem=${BUSD_POOL} --recall-nonce --gas-limit=1000000000 --proxy=${PROXY} --chain=${CHAIN_ID} --function=issue --outfile="issueBUSD.json" --arguments 0X42555344 0X42555344 100000 0 --send
}
...
fundFromPool(){
    erdpy contract call ${ADDRESS} --recall-nonce --gas-limit=1000000000 --proxy=${PROXY} --pem=${BUSD_POOL} --chain=${CHAIN_ID} --function=DCTTransfer --arguments ${DCT_BUSD_TICKER} 50015 0x66756e6446726f6d506f6f6c --send
}

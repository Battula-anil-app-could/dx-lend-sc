    ...
    + common_checks::ChecksModule
{
    ...
    fn add_collateral(
        ...
        let (deposit_asset, deposit_amount) = self.call_value().single_fungible_dct();
    ...
    fn borrow(
        ...
        self.send()
            .direct_dct(&initial_caller, &pool_token_id, 0, &borrow_amount);
    ...
    fn remove_collateral(
        ...
        self.send()
            .direct_dct(&initial_caller, &pool_asset, 0, &withdrawal_amount);
    ...
    fn repay(
        ...
        let (repay_asset, mut repay_amount) = self.call_value().single_fungible_dct();
        let pool_asset = self.pool_asset().get();
        ...
        if repay_amount > total_owed {
            ...
            self.send()
                .direct_dct(&initial_caller, &repay_asset, 0, &extra_amount);
    ...
    fn send_tokens(&self, initial_caller: ManagedAddress, payment_amount: BigUint) {
        ...
        self.send()
            .direct_dct(&initial_caller, &pool_asset, 0, &payment_amount);
    }
    /*
    ...
    fn liquidate(
        ...
        let (liquidator_asset, liquidator_asset_amount) = self.call_value().single_fungible_dct();

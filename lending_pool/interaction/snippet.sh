PEM="~/pems/dev.pem"
ME=moa1n0p3zc0ccr9dptetatlqs950c4cs3lq9gvuwsqtv5ksh8c9qu00sqzevr4
...
# after createPool
LIQ_POOL_ADDRESS=moa1qqqqqqqqqqqqqpgqn8xx3p50927tye5n49nzspvw7qqqayjfu00s8w2fse
...
deposit() {
    moapy contract call ${ADDRESS} --recall-nonce --pem=${PEM} --gas-limit=${GAS_LIMIT} \
    --function="DCTTransfer" --arguments ${ASSET} ${DEPOSIT_AMOUNT} ${DEPOSIT_ENDPOINT} \
    --proxy=${PROXY} --chain=${CHAIN_ID} --send 
}
...
borrow() {
    ADDR_HEX=0x$(moapy wallet bech32 --decode ${ADDRESS})

    moapy contract call ${ME} --recall-nonce --pem=${PEM} --gas-limit=${GAS_LIMIT} --function="DCTNFTTransfer" \
...
get_pool_address() {
    moapy contract query ${ADDRESS} --function="getPoolAddress" --arguments ${ASSET} --proxy=${PROXY}
}

get_pool_address_arg() {
    moapy contract query ${ADDRESS} --function="getPoolAddress" --arguments $1 --proxy=${PROXY}
}

get_ltv() {
    moapy contract query ${ADDRESS} --function="getAssetLoanToValue" --arguments ${ASSET} --proxy=${PROXY}
}

get_liq_bonus() {
    moapy contract query ${ADDRESS} --function="getAssetLiquidationBonus" --arguments ${ASSET} --proxy=${PROXY}
}

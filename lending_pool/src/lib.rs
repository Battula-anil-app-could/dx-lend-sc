    ...
    + price_aggregator_proxy::PriceAggregatorModule
{
    ...
    fn add_collateral(&self) {
        let [nft_account_token, collateral_payment] = self.call_value().multi_dct();
    ...
    #[endpoint]
    fn repay(&self) {
        let [nft_account_token, payment_repay] = self.call_value().multi_dct();
        let (nft_account_token_id, nft_account_nonce, nft_account_amount) =
            nft_account_token.into_tuple();
            ...
            .get(&repay_token_id)
        {
            Some(bp) => {
                let borrow_position: BorrowPosition<Self::Api> = self
                    ...
                    .with_dct_transfer((repay_token_id.clone(), repay_nonce, repay_amount))
                    .execute_on_dest_context();
    ...
    fn liquidate(
        ...
        let (liquidator_asset_token_id, liquidator_asset_amount) =
            self.call_value().single_fungible_dct();
